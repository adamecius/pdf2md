...
def _status_from_counts(c: dict[str, int]) -> str:
    if c["missed"] > 0 or c["ambiguous"] > 0:
        return "fail"
    if c["partial"] > 0:
        return "warn"
    return "pass"
...
def main() -> None:
    ...
    out = Path(a.output)
    gt_by_doc = {
        doc_id_from_tex_path(tex, root): extract_groundtruth_objects(tex.read_text(), doc_id_from_tex_path(tex, root))
        for tex in root.rglob("*.tex")
    }
    backends = a.backend or _detect_backends(root, sorted(gt_by_doc.keys()))
    report = {
        "batch": a.batch,
        "fixture_provenance": sorted(gt_by_doc.keys()),
        "backends": {},
        "evaluation": {},
        "skipped_no_backend_ir": 0,
    }
    total: defaultdict[str, Any] = defaultdict(int)
    for be in backends:
        ...
        for doc_id, gt in gt_by_doc.items():
            ...
            if not has_ir:
                documents[doc_id] = {
                    "status": "skipped_no_backend_ir",
                    "groundtruth_objects": len(gt),
                    "backend_blocks": 0,
                }
                continue
            ...
            for r in recs:
                if r["object_type"] == "equation" and len(r["matched_blocks"]) > 1:
                    r["convention"] = "formula_number_split_block"
                elif r["object_type"] == "table" and (
                    any("cell_text_overlap" in (m.get("match_reason") or "") for m in r["matched_blocks"])
                    or any((m.get("text") or "").lstrip().startswith("Table ") for m in r["matched_blocks"])
                ):
                    r["convention"] = "table_flattened_paragraph"
                elif r["object_type"] == "footnote" and any("1First" in m["text"] for m in r["matched_blocks"]):
                    r["convention"] = "footnote_no_space_after_marker"
                aligns.append(r)
                if r["status"] in {"matched", "partial"} and r.get("convention"):
                    rid = f"{be}.{r['convention']}"
                    pr = proposed.setdefault(
                        rid,
                        {
                            "rule_id": rid,
                            "backend": be,
                            "support": 0,
                            "supporting_gt_ids": set(),
                            "supporting_doc_ids": set(),
                            "supporting_backend_block_ids": set(),
                            "alignment_statuses": set(),
                            "reason": "alignment-derived",
                            "example_before": "",
                            "example_after": "",
                            "groundtruth_source": "latex_fixture+backend_ir",
                        },
                    )
                    pr["support"] += 1
                    pr["supporting_gt_ids"].add(r["gt_id"])
                    pr["supporting_doc_ids"].add(r["doc_id"])
                    pr["alignment_statuses"].add(r["status"])
                    for mb in r["matched_blocks"]:
                        pr["supporting_backend_block_ids"].add(mb["block_id"])
                    if r["matched_blocks"] and not pr["example_before"]:
                        pr["example_before"] = r["matched_blocks"][0]["text"]
                        pr["example_after"] = r["matched_blocks"][0]["text"]
        counts: dict[str, Any] = {
            k: sum(1 for x in aligns if x["status"] == k)
            for k in ["matched", "partial", "missed", "ambiguous", "unsupported"]
        }
        counts["skipped_no_backend_ir"] = sum(1 for d in documents.values() if d["status"] == "skipped_no_backend_ir")
        counts["total_groundtruth_objects"] = len(aligns)
        counts["status"] = _status_from_counts(counts)
        report["backends"][be] = {
            "summary": dict(counts),
            "documents": documents,
            "examples": [
                {
                    ...
                    "groundtruth_object": x["groundtruth_object"],
                    "backend_blocks": x["matched_blocks"],
                    "match_reasons": [m.get("match_reason") for m in x["matched_blocks"]],
                }
                for x in aligns[:30]
            ],
            "alignments": aligns,
            "proposed_rules": [
                {
                    **v,
                    "supporting_gt_ids": sorted(v["supporting_gt_ids"]),
                    "supporting_doc_ids": sorted(v["supporting_doc_ids"]),
                    "supporting_backend_block_ids": sorted(v["supporting_backend_block_ids"]),
                    "alignment_statuses": sorted(v["alignment_statuses"]),
                }
    ...
    total["total_groundtruth_objects"] = sum(
        v["evaluation"]["total_groundtruth_objects"] for v in report["backends"].values()
    )
    total["total_backend_alignments"] = total["total_groundtruth_objects"]
    total["status"] = _status_from_counts(total)
    report["evaluation"] = dict(total)
    report["skipped_no_backend_ir"] = total["skipped_no_backend_ir"]
