...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if loaded.warnings:
            result = type(result)(docling=result.docling, rag_chunks=result.rag_chunks, markdown=result.markdown, manifest=result.manifest.model_copy(update={"warnings": result.manifest.warnings + loaded.warnings}), report={**result.report, "warnings": result.report["warnings"] + loaded.warnings}, warnings=result.warnings + loaded.warnings, write_rag=result.write_rag, write_markdown=result.write_markdown)
