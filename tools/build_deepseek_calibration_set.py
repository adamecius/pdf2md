#!/usr/bin/env python3
"""Run deepseek-ocr on the synthetic LaTeX corpus and drop outputs into
the same ``.tmp/calibration_corpus/`` layout used by the paddleocr loop.

This is the sibling of ``build_paddle_calibration_set.py``: it shares
the same out-root so that ``calibrate_priors.py`` can discover both
backends per document. The two loops can run concurrently because
``pdf2md.backends.deepseek-only.toml`` pins deepseek to physical GPU1
(via ``CUDA_VISIBLE_DEVICES=1``) while paddleocr stays on GPU0.

Usage:
    conda run -n pdf2md python tools/build_deepseek_calibration_set.py \
...
# Reuse the docling -> truth conversion from the paddle script so the two
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--backend-config", type=Path, default=Path("pdf2md.backends.deepseek-only.toml"))
...
def main() -> int:
    ...
    for i, (doc_id, pdf, docling) in enumerate(doc_dirs, start=1):
        ...
        # 1. truth.json — only write if missing so the paddle loop is not
        # disturbed.
        ...
        # 2. deepseek/
        ds_dir = doc_out / "deepseek"
        if ds_dir.is_dir() and (ds_dir / "entities.json").exists() and not args.force:
            skipped.append(doc_id)
            succeeded.append(doc_id)
            continue

        scratch = out_root / "_scratch_deepseek" / doc_id
        if scratch.exists():
            shutil.rmtree(scratch)
        scratch.mkdir(parents=True)
        ok, log_tail = _run_convert(pdf, scratch, args.backend_config, args.timeout)
        connector_src = scratch / "connector" / "deepseek"
        ...
        shutil.copytree(connector_src, ds_dir)
        shutil.rmtree(scratch, ignore_errors=True)
        succeeded.append(doc_id)
    ...
    print()
    print("=" * 70)
    print(f"build_deepseek_calibration_set: {len(succeeded)} succeeded, {len(failed)} failed, {len(skipped)} reused")
