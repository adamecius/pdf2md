...
```text
2026-05-22 — draft — human — Plan 11 prepared in plans/plan-11-entity-proposal-document-validation.md
2026-05-23 — active — feedback — Plan 11 promoted to current_plan.md after Plan 10 archival
2026-05-23 — agent_in_progress — agent — branch plan-11-entity-proposal-document-validation created
2026-05-23 — agent_complete — agent — all 18 required automated tests passed (run_log PR #1, status=ready_for_review)
2026-05-23 — human_verification_required — agent — automated tests complete; human checkpoints H0–H5 staged
2026-05-23 — human_verified — automated review (sandbox) — H1, H2, H3, H4, H5 all pass; H0 deferred because no on-disk real Plan 10 report exists (see Feedback #1)
2026-05-23 — finished — feedback mode — archived and Plan 12 promoted
```
...
- notes:
  - Agent PR #1 (commit `55608cdb`) added exactly the nine whitelisted implementation/test/fixture paths plus `run_log.md`. No conditionally allowed connector files were modified; the existing `EntityProposalDocument` schema, `EntityType`, `RelationType`, `EvidenceKind`, `ConfidenceSource`, and `pdf2md.connectors.common.connect_raw_dir` entrypoint were reused unchanged.
  - All 18 required tests passed (`tests/test_entity_proposal_validation.py`). Full repo suite stayed green: 740 passed, 212 skipped (environmental), 0 failed.
  - Status taxonomy is constrained to the five plan-declared values; relation-endpoint validation and evidence-reference validation are delegated to the repository's existing Pydantic model validators (`EntityProposalDocument._validate_document` and `EntityEvidence._validate_source_block_id`).
  - Gate logic, no-entity rule, semantic-plausibility check, and Plan 12 hand-off summary all behave per spec.
...
- Sandbox script: `/tmp/plan11_human_verification.sh` (evidence: `/tmp/plan11_hv_run/evidence.md`).
- Result: PASS=6, FAIL=0, SKIP=1.
  - H1 (automated tests): PASS — 18/18.
  - H2 (one validated backend): PASS — `valid_entities` fixture validated; `semantic_plausibility_passed=true`, `entity_count=2`, `relation_count=1`, `entity_type_counts={section:2}`, `relation_type_counts={sequence_next:1}`, CLI exit code 0 under `--allow-reduced-gate`, generated `entity_proposals.json` and summary present.
  - H3 (Plan 10 -> Plan 11 chain): PASS — synthetic Plan 10 report (generated by running `tools/validate_connectors_page_ir.py` against the Plan 10 fixture) fed into the Plan 11 CLI; every backend classified inside the five-status taxonomy.
  - H4 (semantic / Plan 12 hand-off + multi-fixture classification): PASS — summary states `Real calibration prior generation is deferred to Plan 12 and is not used for Plan 11 pass/fail`. Multi-fixture run (`valid` + `empty` + `broken`) yields the expected `validated`/`no_entities_produced`/`schema_failed` triple, with `backends_no_entities=1` and `backends_failed=1`.
  - H5 (forbidden-layer diff): PASS — `git diff --name-only main..HEAD` only contains Plan 11 whitelist files plus plan-state files. No calibration, consensus, linking, export, end-to-end, backend execution, or config files were modified.
  - H0 (locate Plan 10 connector validation outputs): SKIP — no on-disk real Plan 10 `connector_validation_report.json` exists (Plan 10 archival did not commit `groundtruth/runs/connector_validation/`). The synthetic Plan 10 report generated in this sandbox drives the same Plan 10 → Plan 11 chain the real CLI would use.
- Reduced-gate approval recorded: not applicable against real backends because no real Plan 10 outputs exist. Approval (synthetic verification accepted as sufficient evidence to unblock Plan 12 work that does not depend on real Plan 10 outputs) recorded here per the human invocation of 2026-05-23.
- Tasks promoted to done: A1, A2, A3, A4, A5 (already promoted by PR_review #1).
- Decision: archive Plan 11 and promote Plan 12 (`plan-12-real-calibration-prior-generation`) to `current_plan.md`. Update `next_plan.md` from the prepared `plans/plan-13-weighted-consensus-ir-real-outputs.md`. Reset `run_log.md` to the empty template for Plan 12.
