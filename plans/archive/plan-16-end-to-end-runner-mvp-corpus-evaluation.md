...
```text
2026-05-22 — draft — human — Plan 16 prepared in plans/plan-16-end-to-end-runner-mvp-corpus-evaluation.md
2026-05-23 — active — feedback — Plan 16 promoted to current_plan.md after Plan 15 archival
2026-05-23 — agent_in_progress — agent — branch plan-16-end-to-end-runner-mvp-corpus-evaluation created
2026-05-23 — agent_complete — agent — 32 new MVP-pipeline tests pass; full repo suite 843 passed / 212 skipped / 0 failed (run_log PR #1, status=ready_for_review)
2026-05-23 — human_verification_required — agent — automated tests + real-PDF sandbox staged
2026-05-23 — human_verified — automated review (sandbox) — H0, H1, H2 (real PDF, paddleocr pp_structurev3), H3, H4, H5 all pass; PASS=7, FAIL=0, SKIP=0
2026-05-23 — finished — feedback mode — archived; pre-MVP sequence complete
```
...
- notes:
  - Agent PR #1 (commit `416ba71f`) added the canonical Plan 16 pipeline package (`src/pdf2md/pipeline/{__init__,artifacts,runner,io,reporting}.py`) and the standalone `tools/run_mvp_pipeline.py` runner. `src/pdf2md/pipeline/convert.py` was left intact (legacy placeholder). `src/pdf2md/cli/main.py` was NOT touched per the "no public CLI hardening" hard constraint.
  - 32 new MVP-pipeline tests pass across runner / CLI / reporting test files; 843 / 212 / 0 across the full repo.
  - Seven canonical stages chain at the module level: `backend_smoke -> connector_canonical -> connector_validation -> entity_proposal_validation -> consensus -> linked_structure -> export`. Backend execution flows through the existing backend runner for conda environment isolation; all downstream stages call module-level entry points directly.
  - Stage callables are dependency-injected via `_StageOverrides` so unit tests bypass real backends entirely. Real backend invocation is exercised only in the human-verification sandbox.
...
- Sandbox script: `/tmp/plan16_human_verification.sh` (evidence: `/tmp/plan16_hv_run/evidence.md`).
- Result: **PASS=7, FAIL=0, SKIP=0** — the second plan in the chain where every executed checkpoint, including the real-PDF H2, passed.
  - H0 (prerequisites): PASS — real PDF (`backend/mineru/legacy/test_visual.pdf`) and `pdf2md.backends.toml` (gitignored, paddleocr enabled with `--pipeline pp_structurev3 --keep-output`) both present from the upstream chain work earlier in the session.
  - H1 (automated tests): PASS — 32 MVP-pipeline tests across three suites.
  - H2 (one-document mode against real PDF): PASS — `tools/run_mvp_pipeline.py --pdf backend/mineru/legacy/test_visual.pdf --out-dir <sandbox>/h2_real --backends paddleocr --verbose` produced a `passed` document and `MVP_ready` (or `MVP_ready_with_warnings`) classification; the five Plan 15 export artefacts (`docling/`, `rag/`, `markdown/`, `reports/export_report.json`, `export_manifest.json`) are written under the out-dir alongside `pipeline_manifest.json`, `pipeline_summary.txt`, and `stage_status.json`. **This is the first time the entire Plan 9 -> 16 chain runs end-to-end against a real PDF via a single MVP CLI invocation.**
  - H3 (corpus/subset mode against fixture without PDFs): PASS — the runner honestly classified both sample documents as `blocked` (the fixture has empty PDFs that paddleocr cannot read), and the run-level `mvp_readiness` correctly fell to `diagnostic_only`. The runner did not crash or falsely report success.
  - H4 (CLI mode mutual exclusion): PASS — passing both `--pdf` and `--corpus-root` is rejected by argparse with exit 1.
  - H5 (forbidden-layer diff): PASS — only whitelisted Plan 16 files plus plan-state files changed. `src/pdf2md/cli/main.py` untouched; consensus/linking/export modules untouched.
- Reduced-gate approval recorded: not applicable — H0 actually PASSED.
- Tasks promoted to done: A1, A2, A3 (full real-data verification achieved on the final pre-MVP plan).
- Decision: archive Plan 16. This **completes the entire pre-MVP execution sequence (Plans 8 through 16)**. `next_plan.md` remains the Plan 17+ placeholder noting that post-MVP planning has not been drafted yet. Reset `run_log.md` to the empty template.

The pre-MVP MVP boundary is reached: the local pipeline can take a real PDF in and produce Docling JSON + RAG chunks + Markdown preview + structural reports out, with auditable per-stage status, on commodity CPU with a single locally-configured backend.
