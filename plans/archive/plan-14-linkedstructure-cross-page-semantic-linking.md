...
```text
2026-05-22 — draft — human — Plan 14 prepared in plans/plan-14-linkedstructure-cross-page-semantic-linking.md
2026-05-23 — active — feedback — Plan 14 promoted to current_plan.md after Plan 13 archival
2026-05-23 — agent_in_progress — agent — branch plan-14-linkedstructure-cross-page-semantic-linking created
2026-05-23 — agent_complete — agent — 110 existing linking tests still pass; 14 new Plan 14 hardening tests added (run_log PR #1, status=ready_for_review with upstream blocker recorded)
2026-05-23 — human_verification_required — agent — automated tests + synthetic baseline complete; human checkpoints H0–H5 staged
2026-05-23 — human_verified — automated review (sandbox) — H1, H2, H3, H4, H5 all pass; H0 against real Plan 13 ConsensusIR deferred upstream (see Feedback #1)
2026-05-23 — finished — feedback mode — archived and Plan 15 promoted
```
...
- tasks_promoted: A1, A2, A3 (synthetic baseline; real-corpus execution deferred upstream)
- notes:
  - Agent PR #1 (commit `1f51453b`) modified exactly four whitelisted files (`src/pdf2md/linking/reporting.py`, `tools/build_linked_structure.py`, `tests/test_build_linked_structure_cli.py`, `run_log.md`). `builder.py`, `extract.py`, `resolvers.py`, `io.py`, and `__init__.py` left untouched — the existing linking pipeline already worked end-to-end on synthetic real-shaped fixtures.
  - Reporting now exposes `link_status_counts`, `relation_type_status` table, `low_confidence` block (configurable threshold), `inputs_used`, `inspection_status` (4-value Plan 15 readiness taxonomy), `inspection_notes`, and `plan15_readiness` (with `docling_export_handled_by="plan_15"`).
  - The CLI accepts `--inspection-status`, `--inspection-note`; defaults to `diagnostic_only`; rejects unknown statuses via argparse choices.
  - Plan 14 hard constraints #13-#15 honoured: only `linked_structure.json` and `reports/linking_report.json` are written; no `linking_summary.txt`, `linked_structure_summary.txt`, or `linked_structure_report.json` is emitted. The CLI prints the readiness summary in `--verbose` mode to stdout only.
  - 110 existing linking tests still pass; 14 new Plan 14 hardening tests pass; full repo suite 801 passed / 212 skipped / 0 failed.
...
- Sandbox script: `/tmp/plan14_human_verification.sh` (evidence: `/tmp/plan14_hv_run/evidence.md`).
- Result: PASS=7, FAIL=0, SKIP=1.
  - H1 (automated tests): PASS — 124 tests across 5 linking test files.
  - H2 (three scenarios): PASS — `simple_document`, `toc_footnotes_references`, and `unresolved_ambiguity` all produced ONLY the two allowed disk outputs (`linked_structure.json` + `reports/linking_report.json`); no `linking_summary.txt`, no `linked_structure_summary.txt`, no `linked_structure_report.json`.
  - H3 (Plan 15 readiness + inspection): PASS — CLI accepted `--inspection-status ready_for_plan_15`, `--inspection-note`; report includes `plan15_readiness.has_reading_order=True`, `entities_root_used=True`, `priors_root_used=True`, `docling_export_handled_by="plan_15"`; full readiness taxonomy enforced.
  - H4 (strict CLI rejection): PASS — unknown inspection status rejected via argparse (exit 2).
  - H5 (forbidden-layer diff): PASS — only whitelisted Plan 14 files + plan-state files changed. Consensus/calibration/connector/export code untouched.
  - H0 (locate real Plan 13 ConsensusIR): SKIP — no on-disk `groundtruth/runs/consensus_one_document/consensus_ir.json`. The synthetic `tests/data/linking_fixtures/{simple_document,toc_footnotes_references,unresolved_ambiguity}` runs exercise the same `build_linked_structure.py` code path the real CLI would use.
- Reduced-gate approval recorded: not applicable against real backends because no real Plan 13 outputs exist. Approval (synthetic verification accepted as sufficient evidence to unblock Plan 15 work that does not depend on real Plan 14 LinkedStructure) recorded here per the human invocation of 2026-05-23.
- Tasks promoted to done: A1 (existing tests pass + linker exercised on synthetic fixture), A2 (reporting + CLI hardened), A3 (Plan 15 readiness + inspection-status taxonomy emitted; real-corpus quality classification remains `diagnostic_only` until upstream outputs become available).
- Decision: archive Plan 14 and promote Plan 15 (`plan-15-docling-export-validation`) to `current_plan.md`. Update `next_plan.md` from the prepared `plans/plan-16-end-to-end-runner-mvp-corpus-evaluation.md`. Reset `run_log.md` to the empty template for Plan 15.
