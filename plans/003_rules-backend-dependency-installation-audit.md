...
- MinerU has focused adapter contract coverage in `tests/test_mineru_backend_contract.py`.

Current backend registration and adapter state:
- `doc2md/backends/registry.py` registers `deterministic`, `mineru`, and `paddleocr_vl`.
- `doc2md/backends/mineru_backend.py` exists as an optional stub with lazy dependency checks.
...
- `envs/` contains backend-specific environment manifests.
- `scripts/` contains backend runner helpers.
- `backend_catalog.yaml` contains backend IDs and registry mappings.
