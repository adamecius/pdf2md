...
def test_semantic_link_table_anchor(tmp_path): _,l,_,_,_=run_pipeline('table_caption_reference',tmp_path); assert sum(1 for a in l['anchors'] if a.get('anchor_type')=='table')>=1
def test_semantic_link_section_reference_resolved(tmp_path): _,l,_,_,_=run_pipeline('section_subsection_references',tmp_path); assert sum(1 for r in l['references'] if r.get('reference_type')=='section' and r.get('resolved'))>=1
def test_semantic_link_footnote_anchor(tmp_path): _,l,_,_,_=run_pipeline('footnotes_basic',tmp_path); assert sum(1 for a in l['anchors'] if a.get('anchor_type')=='footnote')>=1
