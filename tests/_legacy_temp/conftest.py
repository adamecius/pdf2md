...
def built_pipeline_artifacts(tmp_path: Path, sample_pdf_path: Path, backend_ir_root: Path) -> dict:
    ...
    res = run_cli("pdf2md._legacy.consensus_report", str(sample_pdf_path), "--config", str(tmp_config), "--verbose")
    assert res.returncode == 0, res.stderr
    assert cons.exists(), f"Expected consensus output at {cons}"
    res = run_cli("pdf2md._legacy.semantic_linker", str(cons), "--output", str(sem_links), "--verbose")
    assert res.returncode == 0, res.stderr
    res = run_cli("pdf2md._legacy.media_materializer", str(cons), "--semantic-links", str(sem_links), "--output-root", str(out_dir), "--verbose")
    assert res.returncode == 0, res.stderr
    res = run_cli("pdf2md._legacy.semantic_document_builder", str(cons), "--semantic-links", str(sem_links), "--media-manifest", str(media_manifest), "--output", str(sem_doc), "--verbose")
    assert res.returncode == 0, res.stderr
