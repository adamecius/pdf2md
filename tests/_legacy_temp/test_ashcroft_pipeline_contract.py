...
def test_consensus_semantic_media_semanticdoc_contracts(built_pipeline_artifacts: dict, tmp_path: Path):
    ...
    res = run_cli("pdf2md._legacy.media_materializer", str(built_pipeline_artifacts["paths"]["consensus_report"]), "--semantic-links", str(built_pipeline_artifacts["paths"]["semantic_links"]), "--output-root", str(media_dbg_root), "--materialize-orphan-images", "--json-only")
...
def test_docling_adapter_on_fresh_canonical_semantic_document(built_pipeline_artifacts: dict, tmp_path: Path):
    try:
        from pdf2md._legacy.docling_adapter import DoclingBackend, AdapterError
        DoclingBackend.load()
    except Exception:
    ...
    res = run_cli(
        "pdf2md._legacy.docling_adapter",
        str(sem_path),
        "--output-root",
        str(out),
        "--mode",
...
def test_consensus_cli_config_resolution_contract(sample_pdf_path: Path, tmp_path: Path):
    ...
    res_default = run_cli(
        "pdf2md._legacy.consensus_report",
        str(sample_pdf_path),
        "--output",
        str(default_out),
        "--json-only",
