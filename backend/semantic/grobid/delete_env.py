#!/usr/bin/env python3
"""Delete the pdf2md-grobid environment.

What it does:

1. Looks for a Conda environment named pdf2md-grobid.
...
Note: the GROBID source tree downloaded by setup.py lives inside
the conda env's `share/` directory, so removing the env removes
the GROBID install as well. Venv mode does not own the GROBID tree
(it lives wherever the user pointed --grobid-home), so for venv
installs you may need to delete the GROBID directory by hand.
"""
...
def rerun_outside_if_needed(conda_path: Path | None, venvs: list[Path]) -> None:
    if os.environ.get("PDF2MD_GROBID_DELETE_REEXEC") == "1":
        return
    ...
    env["PDF2MD_GROBID_DELETE_REEXEC"] = "1"
