#!/usr/bin/env python3
"""Delete the pdf2md-deepseek-vl2 environment.

What it does:

1. Looks for a Conda environment named pdf2md-deepseek-vl2.
2. Looks for local venv folders:
   - .venv
   - pdf2md-deepseek-vl2
   - .venv-pdf2md-deepseek-vl2
...
Note: the Hugging Face model cache at ~/.cache/huggingface/ is NOT
removed by this script (other envs may share it). Clear it by hand if
you want the ~5.6 GB back.
"""
...
def rerun_outside_if_needed(conda_path: Path | None, venvs: list[Path]) -> None:
    if os.environ.get("PDF2MD_DEEPSEEK_VL2_DELETE_REEXEC") == "1":
        return
    ...
    env["PDF2MD_DEEPSEEK_VL2_DELETE_REEXEC"] = "1"
