...
try:
    from mineru_form_data import build_parse_request_form_data_compat
except Exception:
    from backend.mineru.mineru_form_data import build_parse_request_form_data_compat
...
def collect_files(input_path: str | Path) -> list[Path]:
    ...
    if path.is_dir():
        files = sorted(
            (p.resolve() for p in path.iterdir() if p.is_file() and guess_suffix_by_path(p) in SUPPORTED_SUFFIXES),
            key=lambda x: x.name,
        )
        if not files:
...
async def convert_with_mineru(
    ...
    upload_assets = [_api_client.UploadAsset(path=f, upload_name=f.name) for f in input_files]
    ...
    async with _api_client.get_http_client() as client:
        ...
        try:
            ...
            form_data = build_parse_request_form_data_compat(
                _api_client,
                language=language,
                backend=backend,
                formula=formula,
            ...
            submit_resp = await _api_client.submit_parse_task(base_url=base_url, upload_assets=upload_assets, form_data=form_data)
            await _api_client.wait_for_task_result(client=client, submit_response=submit_resp, task_label=f"{len(input_files)} files")
            zip_path = await _api_client.download_result_zip(client=client, submit_response=submit_resp)
...
def main():
    ...
    parser.add_argument("-b", "--backend", default="hybrid-auto-engine", choices=["hybrid-auto-engine", "pipeline", "vlm-auto-engine"])
